...
public class ItemAdapter extends ArrayAdapter<Item> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final ArrayList<Categories> cat = MainActivity.getCat();
        viewHolder.back.setBackgroundColor(Color.WHITE);
        boolean found = false;
        int i = 0;
        while (i < cat.size()) {
            if (Item.getCategories().equals(cat.get(i).getName())) {
                found = true;
                ...
                if (Item.getStatus() == com.example.todolist.Item.Status.DONE)
